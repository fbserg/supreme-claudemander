...
def _make_credential_manager(states: list[CredentialState] | None = None) -> MagicMock:
...
async def client_with_manager(aiohttp_client, app):
    ...
    # We patch on_startup so it injects our mock instead of starting real tasks.
    ...
    # Suppress real startup side-effects (container probing, session manager, etc.)
    with patch("claude_rts.server.ensure_util_container", new=AsyncMock()), \
        ...
        # Also stub CredentialManager so it doesn't start background tasks
    ...
    # Overwrite what on_startup created with our test manager
    client.app["credential_manager"] = mgr
    return client, mgr


# ── Helper: build a fresh app + client with a given credential manager ────────


async def _make_client(aiohttp_client, mgr: MagicMock):
    """
    Build a minimal aiohttp test client with all real credential routes registered
    and the given mock injected into app['credential_manager'].
    """
    # Use a bare Application so we control every route and avoid real startup tasks.
    bare_app = web.Application()

    # Register only the credential routes (copy from create_app):
...
async def test_credentials_create_dir_failure(aiohttp_client):
    """POST /api/credentials returns 400 when create_profile_dir fails."""
    mgr = _make_credential_manager([])
...
async def test_credentials_delete_dir_failure(aiohttp_client):
    """DELETE /api/credentials/{name} returns 500 when delete_profile_dir fails."""
...
async def test_credentials_probe_unknown_profile(aiohttp_client):
    """POST .../probe works even for profiles not in cache (force_probe will create state)."""
...
async def test_credentials_best_all_stale(aiohttp_client):
    ...
    mgr = _make_credential_manager([state])
    mgr.is_cache_ready.return_value = True
    mgr.get_best.return_value = None  # all stale → no best
    client = await _make_client(aiohttp_client, mgr)
