...
def _make_manager() -> CredentialManager:
    ...
    return CredentialManager(probe_interval=60, health_check_interval=900)
...
async def test_probe_one_happy_path():
    ...
    with patch(f"{_UTIL_MOD}.probe_usage", new=AsyncMock(return_value=mock_usage)), \
...
async def test_probe_one_probe_returns_none():
    """_probe_one() handles probe_usage returning None gracefully."""
    mgr = _make_manager()

    with patch(f"{_UTIL_MOD}.probe_usage", new=AsyncMock(return_value=None)):
...
async def test_probe_one_probe_raises_exception():
    """_probe_one() handles exceptions from probe_usage without crashing."""
    mgr = _make_manager()

    with patch(f"{_UTIL_MOD}.probe_usage", new=AsyncMock(side_effect=RuntimeError("container down"))):
