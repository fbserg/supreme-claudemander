...
async def probe_usage_via_session(
    ...
    """Run claude-usage inside the utility container using a real ConPTY session.

    Uses SessionManager.create_session() to provide a genuine Windows ConPTY,
    which pexpect inside claude-usage requires. Avoids nested PTY issues that
    plague docker exec + script approaches.

    Each probe takes 30-60s; callers should stagger multiple probes.
    """
    ...
    try:
        ...
        if time.monotonic() >= deadline:
            logger.warning(
                "probe_usage_via_session: timed out after {}s for name={}", timeout, name
            )
